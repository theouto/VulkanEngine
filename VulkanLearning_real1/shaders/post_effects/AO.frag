...
#define spacialOffset 0.f
    ...
    {   
        ...
        float dhere = textureLod(depthBuffer, tc_original, 0.0).w;
        ...
        vec3 p1 = ray - GetCameraVec(uv) * textureLod(depthBuffer, uv, 0.0).w;
        
        uv = tc_original + vec2(0.0, viewsizediv.y * normalSampleDist);
        vec3 p2 = ray - GetCameraVec(uv) * textureLod(depthBuffer, uv, 0.0).w;
        
        uv = tc_original + vec2(-viewsizediv.x * normalSampleDist, 0.0);
        vec3 p3 = ray - GetCameraVec(uv) * textureLod(depthBuffer, uv, 0.0).w;
        
        uv = tc_original + vec2(0.0, -viewsizediv.y * normalSampleDist);
        vec3 p4 = ray - GetCameraVec(uv) * textureLod(depthBuffer, uv, 0.0).w;
        ...
        float dirAngle = (PI / 16.0) * (((int(gl_FragCoord.x) + int(gl_FragCoord.y) & 3) << 2) + (int(gl_FragCoord.x) & 3)) + 
                          0.f;//random(vec2(texCoords.x * cos(34534.12313), texCoords.y * 12391.11));
